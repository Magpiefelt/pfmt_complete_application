    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="!showWizard" class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <!-- Legacy Wizard (for Directors/SPMs/Admins) -->
          <Card 
            v-if="canUseLegacyWizard" 
            class="p-6 hover:shadow-lg transition-shadow cursor-pointer" 
            @click="openLegacyWizard"
          >
            <div class="text-center">
              <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center mx-auto mb-4">
                <FileText class="h-6 w-6 text-green-600" />
              </div>
              <h3 class="text-lg font-semibold mb-2">Legacy Wizard (Fast Create)</h3>
              <p class="text-gray-600 mb-4">Power user tool for experienced Directors and SPMs to create projects quickly.</p>
...
// Computed properties
const canUseLegacyWizard = computed(() => {
  const userRole = authStore.currentUser?.role?.toLowerCase()
  return ['director', 'spm', 'admin'].includes(userRole || '')
})
...
// Open legacy wizard for Directors/SPMs/Admins
const openLegacyWizard = () => {
  router.push({ name: 'legacy-wizard-new' })
}
