-- Phase 2 Database Migrations: Project Versions, Calendar Events, and Guidance System (Corrected for UUID)
...
CREATE TABLE IF NOT EXISTS meeting_agendas (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    gate_meeting_id UUID NOT NULL REFERENCES gate_meetings(id) ON DELETE CASCADE,
    title VARCHAR(500) NOT NULL,
    description TEXT,
...
CREATE TABLE IF NOT EXISTS project_workflow_states (
    ...
    next_action_assignee UUID REFERENCES users(id),
...
BEGIN
    INSERT INTO calendar_events (
        ...
        NEW.project_id, 
        'gate_meeting', 
        CONCAT('Gate Meeting: ', COALESCE(NEW.gate_type, 'General')),
        NEW.description,
        NEW.scheduled_date,
        'gate_meetings',
        NEW.id,
...
-- Only create trigger if gate_meetings table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'gate_meetings') THEN
        DROP TRIGGER IF EXISTS gate_meeting_calendar_event ON gate_meetings;
...
-- Function to generate guidance notification
...
BEGIN
    ...
    IF sample_project_id IS NOT NULL AND sample_user_id IS NOT NULL THEN
        ...
        SELECT 
            ...
            'Approved',
            jsonb_build_object(
                'name', COALESCE(p.name, 'Sample Project'),
                'description', COALESCE(p.description, 'Sample project description'),
                'category', COALESCE(p.category, 'Infrastructure'),
                'total_budget', COALESCE(p.total_budget, 1000000),
                'current_budget', COALESCE(p.current_budget, 1000000),
        ...
        SELECT 
            ...
            'Planning',
            sample_user_id,
            'Complete planning documentation and schedule Gate 2 review',
